...
/** After the voice ends, let the ambient bed linger this long, then stop. */
const TAIL_MS = 5_000;
...
export const usePlayerStore = create<PlayerStore>((set, get) => {
  const onStatus = (status: AVPlaybackStatus) => {
    ...
    if (status.didJustFinish) {
      // Voice finished. Unload just the voice but leave the ambient bed playing
      // for a gentle 5s tail, then end the session. Every journey stops on its
      // own — no auto-advance to the next one.
      const v = voice;
      voice = null;
      v?.unloadAsync().catch(() => {});
